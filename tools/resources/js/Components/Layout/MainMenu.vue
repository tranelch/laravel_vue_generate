    ...
    <div class="main-menu-container" :class="menuOpen ? 'menu-open' : ''">
      ...
      <ul class="main-menu p-6 block list-none">
        ...
        <template v-for="item in mainMenuJson" :key="item.title">
          <li v-if="hasPermission(item) && (!item.userCondition || $page.props.user[item.userCondition])" :class="[item.class, item.size, { 'empty' : !item.iconComponent }]">
            <Link :method="item.attributes.method" :as="item.attributes.as" :type="item.attributes.type" :href="item.link" class="flex flex-row" :class="activeClass($page.url, item.link)">
